...
//    passes is the coordinate index
//    M is the number of coordinates
//    x is the array
//    This function adds more memory as needed in 8192 work chunks
//
static void readcoord(char* line,int n,float* x[],int* passes,int* M)
{
...
int LoadOBJ(const char* file)
{
   ...
   while ((line = readline(f)))
   {
      ...
      //  Normal coordinates (always 3)
         ...
         readcoord(line+2,3,&passes,&Nn,&Mn);
      ...
      else if (line[0]=='f')
      {
         ...
         while ((str = getword(&line)))
         {
            ...
            if (Kn) glNormal3fv(passes+3*(Kn-1));
